//USAGE: 	long id = getHardDriveComputerID ();
...
#include   <Tlhelp32.h>
  ...
  MessageBoxA(NULL,   "Create   process   snapshot   failed   !",   "Notice",   MB_ICONINFORMATION   |   MB_OK);
  return   0;
  }
  ...
  MessageBoxA(NULL,   "Process32Frist   function   runs   failed   !",   "Notice",   MB_ICONINFORMATION   |   MB_OK);
  ...
  do   {
    if   (!lstrcmp(wszProcess, pe32.szExeFile))
  ...
  bool   create_windows_shotcut   ( LPCWSTR   szPath,  LPSTR   szLink,LPCWSTR szArguments,LPCWSTR szIcon) //快捷方式的数据文件名(*.lnk)
  {
        HRESULT   hres   ;
        IShellLinkW   *   psl   ;
        IPersistFile*   ppf   ;
